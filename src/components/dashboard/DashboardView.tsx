...
export const DashboardView: React.FC = () => {
  ...
  const toggleRsvp = (annId: string) => {
    setAnnouncements(prev => prev.map(a => {
      if (a.id === annId) {
        const nextStatus = a.rsvpStatus === 'Attending' ? 'Pending' : 'Attending';
        showToast('success', 'RSVP Updated', `You are marked as "${nextStatus}" for ${a.title}`);
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-sm text-stone-600 max-w-2xl leading-relaxed">
              Here is a summary of your classroom's progress, upcoming curriculum deadlines, and special education milestones for today.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-stone-500 uppercase tracking-wider">Today's Attendance</span>
          ...
          <p className="text-xs text-stone-500 mt-2 flex items-center justify-between">
            <span>Grade 1 Sequoia ({presentCount}/{totalAttended})</span>
        ...
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-stone-500 uppercase tracking-wider">Curriculum Status</span>
            <div className="w-8 h-8 rounded-lg bg-amber-50 text-amber-700 flex items-center justify-center">
              <BookOpen className="w-4 h-4" />
            </div>
          </div>
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-3xl font-black text-stone-900">{journeys.length}</span>
            <span className="text-xs font-semibold text-stone-600">Active Units</span>
          </div>
          <p className="text-xs text-stone-500 mt-2 flex items-center justify-between">
        ...
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-stone-500 uppercase tracking-wider">IEP Milestones</span>
            <div className="w-8 h-8 rounded-lg bg-purple-50 text-purple-700 flex items-center justify-center">
              <GraduationCap className="w-4 h-4" />
            </div>
          </div>
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-3xl font-black text-stone-900">{totalGoals - achievedGoals}</span>
            <span className="text-xs font-semibold text-purple-800">In Progress</span>
          </div>
          <p className="text-xs text-stone-500 mt-2 flex items-center justify-between">
        ...
        >
          ...
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-3xl font-black text-stone-900">{students.length}</span>
          ...
          <div className="bg-white border border-[#EFE7DC] rounded-3xl p-6 shadow-xs">
            <div className="flex items-center justify-between pb-4 border-b border-stone-100">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-bold text-stone-900">Today's Academic Schedule</h2>
              </div>
              <span className="text-xs text-stone-500 font-medium">Friday, October 24, 2026</span>
            </div>

            <div className="divide-y divide-stone-100 mt-2">
              {SEED_SCHEDULE_ITEMS.map((item) => (
                <div key={item.id} className="py-3.5 flex items-start gap-4 hover:bg-stone-50/60 p-2 rounded-xl transition-colors">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                    item.type === 'academic' ? 'bg-blue-50 text-blue-800' :
                    item.type === 'pe' ? 'bg-amber-50 text-amber-900' :
                    item.type === 'meeting' ? 'bg-purple-50 text-purple-800' : 'bg-stone-100 text-stone-700'
                  }`}>
          ...
          <div className="bg-white border border-[#EFE7DC] rounded-3xl p-6 shadow-xs">
            <div className="flex items-center justify-between pb-4 border-b border-stone-100">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base font-bold text-stone-900">Current Learning Journeys</h2>
              </div>
              <button
                onClick={() => setActiveTab('LEARNING_JOURNEY_TRACKER')}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              {journeys.slice(0, 2).map((j) => (
                ...
                >
                  <div>
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] font-bold text-stone-600">{j.grade} · {j.subject}</span>
                      <StatusBadge status={j.principalReviewStatus === 'Done' ? 'Approved' : j.principalReviewStatus} size="sm" />
                    </div>
                    <h3 className="text-sm font-bold text-stone-900 mt-1">{j.title}</h3>
                    <p className="text-xs text-stone-600 mt-1 line-clamp-2">
                      {j.projects[0]?.description || 'Comprehensive unit curriculum with cross-curricular connections.'}
                    </p>
                  </div>

                  <div className="pt-2 border-t border-[#E8DFC8]/60 flex items-center justify-between text-xs">
                    <span className="text-stone-500">Author: {j.authorName}</span>
                    <button
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-[#EFE7DC] rounded-3xl p-6 shadow-xs space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-stone-100">
              <h2 className="text-base font-bold text-stone-900">Faculty Announcements</h2>
            ...
            <div className="space-y-4">
              {announcements.map((ann) => (
                <div key={ann.id} className="p-4 rounded-2xl bg-[#FAF5EF] border border-[#E8DFC8] space-y-2">
                  <h3 className="text-xs font-bold text-stone-900 leading-snug">{ann.title}</h3>
                  ...
                  <div className="pt-2 flex items-center justify-between">
                    <span className="text-[11px] text-stone-500">{ann.location}</span>
                    <button
                    ...
                    >
                      {ann.rsvpStatus === 'Attending' ? '✓ Attending' : 'RSVP Now'}
                    </button>
                  </div>
                </div>
              ))}
          ...
          <div className="bg-gradient-to-br from-[#6E161E] to-[#470B10] rounded-3xl p-6 text-white shadow-md space-y-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold uppercase tracking-wider text-[#F5B842]">Special Education Portal</span>
            </div>
            <h3 className="text-lg font-black font-heading leading-tight">
              Individualized Education Program (IEP)
            </h3>
            <p className="text-xs text-stone-200 leading-relaxed">
              Track student sensory profiles, functional emotional developmental capacities (FEDC), and log weekly milestone progress.
            </p>
