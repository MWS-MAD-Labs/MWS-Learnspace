...
const SUBJECT_OPTIONS = ['Physical Education', 'Science', 'Math', 'English', 'Art', 'Music', 'Drama', 'Social Studies', 'Bahasa Indonesia'];
const UNIT_OPTIONS = ['Early Years', 'Elementary', 'Junior High'];
const GRADE_OPTIONS = ['K1', 'K2', 'Grade 1', 'Grade 2', 'Grade 3', 'Grade 4', 'Grade 5', 'Grade 6'];
const MONTH_OPTIONS = ['August 2026', 'September 2026', 'October 2026', 'November 2026', 'December 2026', 'January 2027', 'February 2027', 'March 2027', 'April 2027', 'May 2027', 'June 2027'];
...
export const LearningJourneyEditor: React.FC = () => {
      ...
      projects: [
        {
          ...
          description: 'Students investigate foundational questions, build initial understanding, and collaborate on hands-on inquiry projects.',
          ...
          crossCurricularConnections: [
            { id: `c-${Date.now()}-1`, subject: 'Science', description: 'Investigating observational patterns and scientific methods.' }
          ],
          learningGoals: [
            { id: `g-${Date.now()}-1`, description: 'Demonstrate essential understanding and apply inquiry strategies.', order: 1 }
          ]
        }
      ],
  ...
  const handleAddProject = () => {
    ...
    const newProj: LearningJourneyProject = {
      ...
      crossCurricularConnections: [],
      learningGoals: [
        { id: `g-${Date.now()}`, description: 'Define primary learning outcome...', order: 1 }
      ]
    };
    ...
    showToast('info', 'Project Added', 'New project module added to learning journey.');
  };
  ...
  const handleDeleteProject = (pId: string) => {
    ...
    if (journey.projects.length <= 1) {
      showToast('warning', 'Minimum 1 Project Required', 'A learning journey must contain at least one project.');
      return;
    }
    setJourney(prev => ({
      ...prev,
  ...
  const handleUpdateProjectField = (pId: string, field: keyof LearningJourneyProject, val: any) => {
    if (isLocked) return;
    setJourney(prev => ({
      ...prev,
      projects: prev.projects.map(p => p.id === pId ? { ...p, [field]: val } : p)
    }));
  };
    ...
    setJourney(prev => ({
      ...
      projects: prev.projects.map(p => {
        if (p.id === pId) {
          return {
            ...p,
            learningGoals: p.learningGoals.map(g => g.id === gId ? { ...g, description } : g)
          };
        }
        return p;
      })
    ...
    setJourney(prev => ({
      ...
      projects: prev.projects.map(p => {
        if (p.id === pId) {
          ...
          return { ...p, crossCurricularConnections: [...(p.crossCurricularConnections || []), newConn] };
        }
        return p;
      })
    }));
  ...
  // Update Connection
  const handleUpdateConnection = (pId: string, cId: string, field: 'subject' | 'description', val: string) => {
    if (isLocked) return;
    setJourney(prev => ({
      ...prev,
      projects: prev.projects.map(p => {
        if (p.id === pId) {
          return {
            ...p,
            crossCurricularConnections: p.crossCurricularConnections.map(c => c.id === cId ? { ...c, [field]: val } : c)
          };
        }
        return p;
      })
    ...
    setJourney(prev => ({
      ...
      projects: prev.projects.map(p => {
        if (p.id === pId) {
          return {
            ...p,
            crossCurricularConnections: p.crossCurricularConnections.filter(c => c.id !== cId)
          };
        }
        return p;
      })
  ...
  const handleSaveDraft = () => {
    if (isLocked) {
      showToast('error', 'Action Locked', 'This journey is locked and cannot be saved.');
      return;
    }
    ...
    showToast('success', 'Draft Saved', 'Your learning journey draft has been persisted.');
    refreshData();
  };
  ...
  const handleSubmitForReview = () => {
    if (isLocked) {
      showToast('error', 'Action Locked', 'This journey is already submitted or approved.');
      return;
    }
    ...
    showToast('success', 'Submitted for Review', 'Learning journey successfully forwarded to Principal review queue.');
    refreshData();
    setActiveTab('LEARNING_JOURNEY_TRACKER');
  };

  const returnedHistory = journey.workflowHistory?.find(w => w.action === 'Returned');

  return (
    <div id="learning-journey-editor-view" className="space-y-8 max-w-5xl mx-auto pb-28">
        ...
        <div className="p-4 rounded-2xl bg-stone-100 border border-stone-300 shadow-xs flex items-start gap-3">
          ...
          <div>
            <h3 className="text-xs font-bold text-stone-900 uppercase tracking-wider">
              {isApproved ? 'Approved Curriculum — Locked Against Editing' : 'Submitted for Review — Locked Against Editing'}
            </h3>
        ...
        <div className="p-5 rounded-2xl bg-rose-50 border border-rose-200 shadow-xs flex items-start gap-3">
          ...
          <div className="space-y-1">
            <h3 className="text-xs font-bold text-rose-900 uppercase tracking-wider">
              Revision Requested by {returnedHistory.userName} ({returnedHistory.userRole})
            </h3>
            <p className="text-xs text-rose-800 leading-relaxed font-medium">
              "{returnedHistory.comment}"
            </p>
            <p className="text-[11px] text-rose-600">
              Please make the required changes below and click "Resubmit for Review" at the bottom of the page.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-1.5">
            ...
            <input
              ...
              onChange={(e) => setJourney({ ...journey, academicYear: e.target.value })}
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              onChange={(e) => setJourney({ ...journey, semester: e.target.value as any })}
              className={`w-full px-3.5 py-2 text-xs border rounded-xl font-semibold ${
                isLocked 
                  ? 'bg-stone-100 border-stone-200 text-stone-600 cursor-not-allowed' 
                  : 'bg-[#FAF5EF] border-[#E8DFC8] text-stone-900'
          ...
          <div className="space-y-1.5">
            ...
            <select
              disabled={isLocked}
              value={journey.grade}
              onChange={(e) => setJourney({ ...journey, grade: e.target.value })}
              className={`w-full px-3.5 py-2 text-xs border rounded-xl font-semibold ${
                isLocked 
                  ? 'bg-stone-100 border-stone-200 text-stone-600 cursor-not-allowed' 
                  : 'bg-[#FAF5EF] border-[#E8DFC8] text-stone-900'
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              onChange={(e) => setJourney({ ...journey, subject: e.target.value })}
              className={`w-full px-3.5 py-2 text-xs border rounded-xl font-semibold ${
                isLocked 
                  ? 'bg-stone-100 border-stone-200 text-stone-600 cursor-not-allowed' 
                  : 'bg-[#FAF5EF] border-[#E8DFC8] text-stone-900'
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-stone-500">
                      {project.startMonth} – {project.endMonth} · {project.learningGoals.length} Learning Goals
                    </p>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="md:col-span-1 space-y-1.5">
                      ...
                      <input
                        ...
                        className={`w-full px-3.5 py-2 text-xs border rounded-xl font-bold ${
                          isLocked ? 'bg-stone-100 border-stone-200 text-stone-600' : 'bg-[#FAF5EF] border-[#E8DFC8] text-stone-900'
                        }`}
                      />
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <select
                        ...
                        onChange={(e) => handleUpdateProjectField(project.id, 'startMonth', e.target.value)}
                        className={`w-full px-3.5 py-2 text-xs border rounded-xl font-semibold ${
                          isLocked ? 'bg-stone-100 border-stone-200 text-stone-600' : 'bg-[#FAF5EF] border-[#E8DFC8] text-stone-900'
                        }`}
                      >
                    ...
                    <div className="space-y-1.5">
                      ...
                      <select
                        ...
                        onChange={(e) => handleUpdateProjectField(project.id, 'endMonth', e.target.value)}
                        className={`w-full px-3.5 py-2 text-xs border rounded-xl font-semibold ${
                          isLocked ? 'bg-stone-100 border-stone-200 text-stone-600' : 'bg-[#FAF5EF] border-[#E8DFC8] text-stone-900'
                        }`}
                      >
                    ...
                    {project.crossCurricularConnections?.length === 0 ? (
                      ...
                      <div className="space-y-2.5">
                        {project.crossCurricularConnections.map((conn) => (
                          <div key={conn.id} className="flex flex-col sm:flex-row items-start gap-2 bg-[#FAF5EF] p-2.5 rounded-xl border border-[#E8DFC8]">
                            <select
                              disabled={isLocked}
                              value={conn.subject}
                              onChange={(e) => handleUpdateConnection(project.id, conn.id, 'subject', e.target.value)}
                            ...
                            <input
                              ...
                              onChange={(e) => handleUpdateConnection(project.id, conn.id, 'description', e.target.value)}
                            ...
                            {!isLocked && (
                              <button
                                type="button"
                                onClick={() => handleRemoveConnection(project.id, conn.id)}
                  ...
                  <div className="space-y-3 pt-2">
                    ...
                    <div className="space-y-2">
                      {project.learningGoals?.map((goal, gIndex) => (
                        <div key={goal.id || gIndex} className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            onChange={(e) => handleUpdateGoal(project.id, goal.id, e.target.value)}
                          ...
                          {!isLocked && project.learningGoals.length > 1 && (
                            <button
                              type="button"
                              onClick={() => handleRemoveGoal(project.id, goal.id)}
                              className="p-1.5 text-stone-400 hover:text-rose-600 rounded-lg transition-colors"
                            >
                              <Trash2 className="w-4 h-4" />
                            </button>
          ...
          {isLocked ? (
            ...
            <>
              ...
              >
                ...
                {isReturned ? 'Resubmit for Principal Review' : 'Submit for Principal Review'}
              </button>
            </>
          )}
        </div>
            ...
            <div className="flex items-start justify-between pb-3 border-b border-stone-100">
              <div>
                ...
                <p className="text-xs text-stone-500 mt-0.5">
                  Academic Year: {journey.academicYear} · {journey.semester} · Author: {journey.authorName}
                </p>
              </div>
              <button
                onClick={() => setIsPreviewOpen(false)}
              ...
              {journey.projects.map((proj, idx) => (
                <div key={proj.id} className="p-4 bg-[#FAF5EF] rounded-2xl border border-[#E8DFC8] space-y-3">
                  ...
                  {proj.learningGoals.length > 0 && (
                    <div className="pt-2">
                      <span className="text-xs font-bold text-stone-900">Learning Goals:</span>
                      <ul className="mt-1 space-y-1">
                        {proj.learningGoals.map((g, gi) => (
                          <li key={g.id} className="text-xs text-stone-600 flex items-start gap-1.5">
