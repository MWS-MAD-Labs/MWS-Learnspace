...
type CoordinatorTab = 'FORMS' | 'ASSIGNMENTS' | 'GPK_MANAGEMENT' | 'ALL_RESULTS';
...
export const CoordinatorObservationManager: React.FC = () => {
  ...
  // Forms state
  const [forms, setForms] = useState<ObservationFormDefinition[]>(() => storageService.getObservationForms());
  const [editingForm, setEditingForm] = useState<ObservationFormDefinition | null>(null);

  // Assignments state
  const [assignments, setAssignments] = useState<ObservationAssignment[]>(() => storageService.getObservationAssignments());
  const [isAssignModalOpen, setIsAssignModalOpen] = useState(false);
  const [newAssignment, setNewAssignment] = useState({
    studentId: students.filter(s => s.specialNeedsFlag)[0]?.id || students[0]?.id || '',
    assignedToUserId: allUsers.find(u => u.roleTitle.includes('Therapist') || u.isGPK)?.id || allUsers[0]?.id || '',
  ...
  const [selectedStudentForGPK, setSelectedStudentForGPK] = useState<Student | null>(null);
  ...
  const activeStudentForResults = students.find(s => s.id === selectedStudentId) || specialStudents[0] || students[0];
  ...
  const gpkTeachers = allUsers.filter(u => u.isGPK || (u.roleTitle.toLowerCase().includes('gpk') && !u.isSpecialEdCoordinator));
  ...
  const handleSaveForm = (form: ObservationFormDefinition) => {
    ...
    setEditingForm(null);
    showToast('success', 'Observation Form Template Saved', `Updated configuration for "${form.title}"`);
  };
  ...
  const handleCreateAssignment = (e: React.FormEvent) => {
    ...
    const assignedUser = allUsers.find(u => u.id === newAssignment.assignedToUserId);

    if (!student || !assignedUser) {
      showToast('error', 'Incomplete Assignment', 'Please select a valid student and assessor.');
      return;
    }
    ...
    showToast('success', 'Observation Assigned', `Assigned ${created.instrumentType} for ${student.name} to ${assignedUser.name}`);
  };
  ...
  const handleDeleteAssignment = (id: string) => {
    ...
    setAssignments(storageService.getObservationAssignments());
    showToast('info', 'Assignment Removed', 'The observation assignment was deleted.');
  };
  ...
  const handleAssignGPK = (e: React.FormEvent) => {
    ...
    const currentTeacherAssignedStudents = students.filter(
      s => s.assignedGPKTeacherId === selectedGPKTeacherId && s.id !== selectedStudentForGPK.id
    );
    ...
    if (success) {
      ...
      showToast('error', 'Assignment Failed', 'Maximum 2 students allowed per GPK teacher.');
    }
  };
  ...
  if (!isAuthorizedCoordinator) {
    return (
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 text-center max-w-lg mx-auto my-8">
        ...
        <h3 className="font-bold text-stone-900 text-base">Special Education Coordinator Access Required</h3>
        <p className="text-xs text-stone-600 mt-1">
          Observation form template administration, staff assignments, and GPK caseload allocation are managed by the Special Education Coordinator.
        </p>
      </div>
    );
  }
      ...
      <div className="bg-white border border-[#EFE7DC] rounded-2xl p-4 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-stone-500 mt-0.5">
            Configure observation forms, assign specialist evaluations, manage GPK caseloads, and inspect results.
          </p>
        </div>
        ...
        <div id="gpk-management-section" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {gpkTeachers.map(teacher => {
              const assigned = students.filter(s => s.assignedGPKTeacherId === teacher.id);
              ...
              return (
                <div 
                  ...
                  className={`bg-white border rounded-2xl p-4 shadow-xs space-y-3 transition-all ${
                    isFull ? 'border-amber-300 bg-amber-50/20' : 'border-[#EFE7DC]'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      <img
                        src={teacher.avatarUrl || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=80'}
                        alt={teacher.name}
                        className="w-9 h-9 rounded-xl object-cover border border-stone-200"
                      />
                      <div>
                        <h4 className="font-bold text-xs text-stone-900">{teacher.name}</h4>
                        <p className="text-[10px] text-stone-500">{teacher.roleTitle}</p>
                      </div>
                    </div>
                  ...
                  <div className="pt-2 border-t border-stone-100 space-y-1.5 text-xs">
                    <span className="text-[10px] font-bold text-stone-400 uppercase">Assigned Special Needs Students:</span>
                    {assigned.length === 0 ? (
                      <p className="text-[11px] text-stone-400 italic">No students assigned currently (Capacity: 2 available)</p>
                    ) : (
                      assigned.map(s => (
                        <div key={s.id} className="flex items-center justify-between bg-[#FAF5EF] p-2 rounded-lg border border-[#E8DFC8]">
                          <span className="font-semibold text-stone-800 text-xs">{s.name} (Gr {s.grade})</span>
            ...
            <div className="p-4 border-b border-[#EFE7DC] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-[#FAF5EF]">
              <div>
                ...
                <p className="text-xs text-stone-500">
                  Rule: Each GPK Teacher can support a maximum of 2 Special Needs students.
                </p>
              </div>
            </div>
              ...
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-[#EFE7DC] text-stone-800">
                  {specialStudents.map(student => {
                    const assignedTeacher = allUsers.find(u => u.id === student.assignedGPKTeacherId);
                    return (
                      <tr key={student.id} className="hover:bg-[#FAF5EF]/60 transition-colors">
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-stone-400 font-mono">NISN: {student.nisn}</p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="py-3 px-4">
                          {assignedTeacher ? (
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <p className="font-bold text-stone-900">{assignedTeacher.name}</p>
                                <p className="text-[10px] text-stone-500">{assignedTeacher.roleTitle}</p>
                              </div>
                            </div>
                          ) : (
                        ...
                        <td className="py-3 px-4 text-right">
                          <button
                            ...
                            onClick={() => {
                              ...
                              setSelectedGPKTeacherId(student.assignedGPKTeacherId || gpkTeachers[0]?.id || '');
        ...
        <div id="observation-assignments-section" className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-stone-500">
                Delegate annual or ad-hoc developmental evaluations to Occupational Therapists, Speech Pathologists, or GPKs.
              </p>
            </div>
              ...
              >
                <div className="flex items-center justify-between">
                  <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold ${
                    assn.instrumentType === 'FEDC' ? 'bg-purple-100 text-purple-800' :
                    assn.instrumentType === 'SENSORY_PROFILE' ? 'bg-emerald-100 text-emerald-800' :
                  ...
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                    assn.status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-800' :
                    assn.status === 'IN_PROGRESS' ? 'bg-amber-100 text-amber-800' :
                ...
                <div>
                  <h4 className="font-heading font-bold text-sm text-stone-900">{assn.studentName}</h4>
                  <p className="text-xs text-stone-600 mt-0.5">
                    Assigned to: <strong className="text-stone-900">{assn.assignedToUserName}</strong> ({assn.assignedToUserRole})
                  </p>
                </div>

                <div className="bg-[#FAF5EF] p-2.5 rounded-xl border border-[#E8DFC8] text-[11px] space-y-1 text-stone-600">
                  <p><strong className="text-stone-800">Due Date:</strong> {assn.dueDate}</p>
                  <p><strong className="text-stone-800">Academic Year:</strong> {assn.academicYear}</p>
                  <p><strong className="text-stone-800">Notes:</strong> {assn.notes}</p>
                </div>

                <div className="flex items-center justify-between pt-2 border-t border-stone-100 text-[11px]">
                  <span className="text-stone-400">Assigned {assn.assignedDate}</span>
                  <button
        ...
        <div id="observation-forms-section" className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-stone-500">
                Manage evaluation criteria, score ranges, and diagnostic instruments for the Special Education program.
              </p>
            </div>
            ...
            <button
              ...
              onClick={() => setEditingForm({
                ...
                type: 'CUSTOM',
                version: '1.0',
                description: 'Custom diagnostic rubric for classroom observation.',
                sectionsCount: 4,
                totalItemsCount: 20,
                maxScore: 60,
            ...
            {forms.map(form => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] font-mono text-stone-400">v{form.version}</span>
                </div>

                <div>
                  <h4 className="font-heading font-bold text-sm text-stone-900">{form.title}</h4>
                  <p className="text-xs text-stone-500 mt-1 leading-relaxed">{form.description}</p>
                </div>

                <div className="grid grid-cols-3 gap-2 bg-[#FAF5EF] p-2.5 rounded-xl border border-[#E8DFC8] text-center text-xs">
                  <div>
                    <span className="text-[10px] text-stone-400 uppercase font-bold">Sections</span>
                    <p className="font-bold text-stone-800">{form.sectionsCount}</p>
                  </div>
                  <div>
                    <span className="text-[10px] text-stone-400 uppercase font-bold">Items</span>
                    <p className="font-bold text-stone-800">{form.totalItemsCount}</p>
                  </div>
                  <div>
                    <span className="text-[10px] text-stone-400 uppercase font-bold">Max Score</span>
                    <p className="font-bold text-stone-800">{form.maxScore}</p>
                  </div>
                </div>

                <div className="flex items-center justify-between pt-2 border-t border-stone-100">
                  <span className="text-[10px] text-stone-400">Updated: {form.lastUpdated}</span>
                  <button
        ...
        <div id="all-results-section" className="space-y-6">
          ...
          <div className="bg-white border border-[#EFE7DC] rounded-2xl p-4 shadow-xs">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div>
                <span className="text-[10px] font-bold text-stone-500 uppercase">Coordinator Student Explorer</span>
                <h3 className="font-heading font-bold text-sm text-stone-900">Select Special Needs Student:</h3>
              </div>
              ...
              <div className="flex items-center gap-2 overflow-x-auto pb-1 max-w-full">
                {specialStudents.map(s => (
                  ...
                  >
                    <img
                      src={s.avatarUrl || 'https://images.unsplash.com/photo-1544717305-2782549b5136?w=60'}
                      alt={s.name}
        ...
        <div className="fixed inset-0 z-50 bg-stone-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-stone-200 space-y-5 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-stone-100 pb-3">
              <div>
                <h3 className="font-heading font-black text-base text-stone-900">Assign GPK Teacher</h3>
                <p className="text-xs text-stone-500 mt-0.5">Student: {selectedStudentForGPK.name} (Grade {selectedStudentForGPK.grade})</p>
              </div>
              <button 
            ...
            <form onSubmit={handleAssignGPK} className="space-y-4 text-xs">
              <div className="space-y-1.5">
                <label className="font-bold text-stone-700">Select GPK Teacher (Max 2 Students / GPK):</label>
                <div className="space-y-2">
                  {gpkTeachers.map(teacher => {
                    const assigned = students.filter(s => s.assignedGPKTeacherId === teacher.id && s.id !== selectedStudentForGPK.id);
                    const isFull = assigned.length >= 2;
                    return (
                      <label 
                        key={teacher.id}
              ...
              <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-amber-900 text-[11px] space-y-1">
                ...
                <p>
                  Upon assignment, this teacher will have exclusive direct access to {selectedStudentForGPK.name}'s observation records, IEP plan, and weekly logs.
                </p>
              </div>
      ...
      {isAssignModalOpen && (
        <div className="fixed inset-0 z-50 bg-stone-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-stone-200 space-y-5 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-stone-100 pb-3">
              <div>
                <h3 className="font-heading font-black text-base text-stone-900">Assign Observation Task</h3>
                <p className="text-xs text-stone-500 mt-0.5">Delegate developmental evaluation to specialist</p>
              </div>
              <button 
            ...
            <form onSubmit={handleCreateAssignment} className="space-y-4 text-xs">
              <div className="space-y-1">
                <label className="font-bold text-stone-700">Special Needs Student:</label>
                <select
                ...
                >
                  {specialStudents.map(s => (
                    <option key={s.id} value={s.id}>{s.name} (Grade {s.grade} - {s.primaryDiagnosis})</option>
                  ))}
                </select>
              </div>

              <div className="space-y-1">
                <label className="font-bold text-stone-700">Assign To Assessor / Specialist:</label>
                <select
                  value={newAssignment.assignedToUserId}
                  onChange={(e) => setNewAssignment({ ...newAssignment, assignedToUserId: e.target.value })}
                  className="w-full p-2.5 bg-[#FAF5EF] border border-[#E8DFC8] rounded-xl font-semibold"
                >
                  {allUsers.map(u => (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="font-bold text-stone-700">Instrument Type:</label>
                  <select
                    value={newAssignment.instrumentType}
                    onChange={(e) => setNewAssignment({ ...newAssignment, instrumentType: e.target.value as any })}
                    className="w-full p-2.5 bg-[#FAF5EF] border border-[#E8DFC8] rounded-xl font-semibold"
                  >
              ...
              <div className="space-y-1">
                <label className="font-bold text-stone-700">Evaluation Instructions & Scope:</label>
                <textarea
                  rows={3}
        ...
        <div className="fixed inset-0 z-50 bg-stone-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-stone-200 space-y-5 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-stone-100 pb-3">
              <div>
                <h3 className="font-heading font-black text-base text-stone-900">
                  {editingForm.id.startsWith('form-') ? 'Create Observation Form' : 'Edit Observation Form Template'}
                </h3>
                <p className="text-xs text-stone-500 mt-0.5">Customize rubric fields and scoring bounds</p>
              </div>
              <button 
            ...
            <div className="space-y-4 text-xs">
              ...
              <div className="space-y-1">
                <label className="font-bold text-stone-700">Description / Diagnostic Framework:</label>
                <textarea
                  rows={2}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-1">
                  <label className="font-bold text-stone-700">Sections Count:</label>
                  <input
                    type="number"
                    value={editingForm.sectionsCount}
                    onChange={(e) => setEditingForm({ ...editingForm, sectionsCount: Number(e.target.value) })}
                    className="w-full p-2.5 bg-[#FAF5EF] border border-[#E8DFC8] rounded-xl font-bold"
                  />
                </div>
                <div className="space-y-1">
                  <label className="font-bold text-stone-700">Items Count:</label>
                  <input
                    type="number"
                    value={editingForm.totalItemsCount}
                    onChange={(e) => setEditingForm({ ...editingForm, totalItemsCount: Number(e.target.value) })}
                    className="w-full p-2.5 bg-[#FAF5EF] border border-[#E8DFC8] rounded-xl font-bold"
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => setEditingForm({ ...editingForm, maxScore: Number(e.target.value) })}
                    className="w-full p-2.5 bg-[#FAF5EF] border border-[#E8DFC8] rounded-xl font-bold"
                  />
                </div>
              </div>
