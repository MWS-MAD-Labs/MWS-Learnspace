...
- **Completed (2026-08-26):** Added strict versioned submit, Principal decision, and Director decision contracts that reject actor, organization, and source/target-state spoofing; explicit organization-scoped command endpoints; conditional tenant/journey/source-state/version updates with atomic version increments; immutable workflow and audit events in the same transaction with session-derived actors; creator/owner submission authorization plus `journey:review` and `journey:approve` decisions; API-projected workflow history and returned-review feedback; re-enabled editor/tracker transition controls with saving, validation, denial, and stale-version states; and removal of `storageService.updateWorkflowStage()`. Unit and PostgreSQL integration coverage validates the complete transition table, invalid states, same-tenant non-owner and cross-tenant denial, actor spoofing, stale and simultaneous commands, persisted return comments, and transaction failure behavior.
